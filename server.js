...
app.post('/api/auth/google', asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            // FIX: Handle duplicate username by appending a number suffix if the email-prefix username is taken
        ...
        // FIX: Return full user data so client has accurate credits/stats
        res.json({
            success: true,
            email: user.email,
...
app.post('/api/user-activity', asyncHandler(async (req, res) => {
    ...
    if (user) {
        ...
        // FIX: Only count reasonable diffs (< 10 min) to avoid huge spikes on reconnect
        if (diff < (10 / 60)) {
...
app.post('/api/modules', asyncHandler(async (req, res) => {
    // FIX: Use upsert to prevent duplicate key crash if same id is posted twice
...
// FIX: Added single-bug delete by id (used by admin panel to remove individual reports)
...
// FIX: Added single-ban delete by id (used by admin panel approve/ignore actions)
...
app.post('/api/external/chat', asyncHandler(async (req, res) => {
    ...
    const allKeys = await UserApiKey.find({});
    ...
    try {
        ...
        // FIX: Properly find and update the key usage atomically using findOneAndUpdate
        found.used++;
        await UserApiKey.updateOne(
            { email: foundUser, 'keys.key': token },
            { $set: { 'keys.$.used': found.used, 'keys.$.date': today } }
        );
...
app.post('/api/admin/log-abuse', asyncHandler(async (req, res) => {
    ...
    await log.save();
    // Keep only the last 100 abuse logs
